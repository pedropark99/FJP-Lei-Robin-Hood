...
quantidade_idx <- list.files()[str_detect(list.files(), "Idx")]
...
numero_linhas_ICMS <- nrow(portaria_ICMS)


if(!numero_linhas_ICMS == 853){
  
  print(numero_linhas_ICMS)
  stop("A portaria de ICMS não possui 853 linhas. Neste caso, ou você esqueceu da linha referente a algum município durante o processo de preparação do arquivo da portaria, ou a SEF se esqueceu de incluir as informações de algum município. De qualquer forma, você precisa definir as linhas de todos os municípios, mesmo que você tenha que preencher com $0 reais nas colunas de valores monetários.")
}

numero_linhas_IPI <- nrow(portaria_IPI)


if(!numero_linhas_IPI == 853){
  
  print(numero_linhas_IPI)
  stop("A portaria de IPI não possui 853 linhas. Neste caso, ou você esqueceu da linha referente a algum município durante o processo de preparação do arquivo da portaria, ou a SEF se esqueceu de incluir as informações de algum município. De qualquer forma, você precisa definir as linhas de todos os municípios, mesmo que você tenha que preencher com $0 reais nas colunas de valores monetários.")
}
...
#### Conferindo os tipos de dados contidos nas colunas essenciais das portarias
## Portaria de ICMS
teste_tipos <- portaria_ICMS %>% 
  select(Ano, starts_with("Bruto"), Compensações, FUNDEB, SEF)
  
teste_tipos <- map_lgl(teste_tipos, is.double)
...
colunas_double <- c(
  ...
  "FUNDEB",
  "SEF",
  colnames(portaria_ICMS)[str_detect(colnames(portaria_ICMS), "Bruto")]
)

if(!all(teste_tipos)){
  portaria_ICMS <- portaria_ICMS %>% 
    mutate(across(.cols = all_of(colunas_double), .fns = as.double))
  
}

## Portaria de IPI
teste_tipos <- portaria_IPI %>% 
  select(Ano, PASEP, FUNDEB, Bruto, SEF)
