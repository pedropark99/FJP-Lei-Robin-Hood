...
pesos <- read_csv2("Pesos_ICMS_IPI.csv")

portaria_ICMS <- read_excel("Portaria_ICMS.xlsx")

portaria_IPI <- read_excel("Portaria_IPI.xlsx")



#### Executando o script que confere os nomes das colunas em cada portaria ---------------------------

source("conferir_portarias.R", encoding = "UTF-8")
...
arquivos_idx <- list.files()[str_detect(list.files(), "Idx")]

lista_idx <- map(arquivos_idx, ~read_excel(.) %>% arrange(IBGE2))
...
source("conferir_idx.R", encoding = "UTF-8")
...
calc_novos_indices_ICMS <- function(mes_a_ajustar){
  ...
  ICMS_total_do_mes <- sum(transf$transf, na.rm = T)
...
calc_novos_indices_IPI <- function(){
  ...
  IPI_total_do_mes <- sum(transf$Liquido_IPI, na.rm = T)
...
IPI_Liq_total <- sum(portaria_IPI$Liquido_IPI, na.rm = T)
